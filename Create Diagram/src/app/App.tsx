...
const NODES: FNode[] = [
  { id:"start",           x:330, y:44,   w:220, h:48, label:"[시작] 로그인",          sub:"Trainer ID / Member ID",                     style:"blue" },
      ...
      <div className="flex-1 overflow-auto">
        <div className="min-w-max px-6 py-6">
          <svg width={SVG_W} height={SVG_H} className="rounded-xl overflow-visible">
            ...
            {/* Section label: Trainer + Member */}
            <text x={SVG_W/2} y={20} textAnchor="middle" fontSize={11} fontWeight={600}
              fill="#1e293b" fontFamily="Inter,sans-serif">
              FitNote Trainer — 회원 프로필 준비 + 세션 처리 통합 흐름
            </text>

            {/* ── Edges ── */}
            {EDGES.map((e, i) => <Arrow key={i} d={e.d} />)}
